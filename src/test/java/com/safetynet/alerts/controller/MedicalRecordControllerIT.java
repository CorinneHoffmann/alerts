...
class MedicalRecordControllerIT {
	...
	void shouldGetMedicalRecordWhenEmpty() throws Exception {
		...
		MvcResult result = mockmvc.perform(get("/medicalrecord")).andExpect(status().isOk()).andDo(print()).andReturn();
	}
	...
	void shouldUpdatePerson() throws Exception {
		...
		MvcResult result = mockmvc.perform(put("/medicalrecord").contentType(MediaType.APPLICATION_JSON)
				.content(medicalRecordJson).characterEncoding("utf-8")).andExpect(status().isOk()).andDo(print())
				.andReturn();
	}
	...
	void shouldNotUpdatePersonBecauseNotFound() throws Exception {
		...
		MvcResult result = mockmvc.perform(put("/medicalrecord").contentType(MediaType.APPLICATION_JSON)
				.content(medicalRecordJson).characterEncoding("utf-8")).andExpect(status().isNotFound()).andDo(print())
				.andReturn();
	}
	...
	void shouldDeleteMedicalRecord() throws Exception {
		...
		MvcResult result = mockmvc.perform(delete("/medicalrecord").param("firstName", "Totocreate")
				.param("lastName", "Totocreate name").characterEncoding("utf-8")).andExpect(status().isOk())
	...
	void shouldNotDeleteMedicalRecordBecauseNotFound() throws Exception {
		...
		MvcResult result = mockmvc.perform(delete("/medicalrecord").param("firstName", "Toto1")
				.param("lastName", "Toto1 name").characterEncoding("utf-8")).andExpect(status().isNotFound())
				.andDo(print()).andReturn();
	}
	...
	void shouldNotDeleteMedicalRecordBecauseNotParam() throws Exception {
		...
		MvcResult result = mockmvc
				.perform(delete("/medicalrecord").param("firstName", "Totocreate").characterEncoding("utf-8"))
	...
	void shouldNotFindCompletePath() throws Exception {
		...
		MvcResult result = mockmvc.perform(get("/medicalRecord")).andExpect(status().isNotFound()).andDo(print())
				.andReturn();
	}
	...
	void shouldNotCreateExistingMedicalRecord() throws Exception {
		...
		this.mockmvc.perform(post("/medicalrecord").contentType(MediaType.APPLICATION_JSON).content(medicalRecordJson))
				.andExpect(status().isFound()).andDo(print()).andReturn();
	}
